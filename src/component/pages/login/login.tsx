...
class LoginComponent extends BaseComponent<IProps, IState> {
  ...
  private validateForm(currentInput_isValid: boolean, inputType: string): boolean {
    let step_inputList: string[] = ["userName"];
  ...
  private async onLogin() {
    ...
    let authObj = {
      ...
      password: "123456"//this.state.inputs.password.value!
    }
    try {
  ...
  private async getProfile() {
    try {
      ...
      this.props.history.push(EROUT.NOTES);
    } catch (error) {
      ...
      <>
        <div className="login-wrapper">
          <div className="login-item text-center">
            <img src="" alt="" />
            <h6>wellcome to Nevis</h6>
            <div className="label">Enter e-mail or Phone</div>
          </div>
          <div className="login-item">
            <AppInput value={this.state.inputs.userName.value} onChange={(e) => this.handleInput(e.target.value, "userName")} />
          </div>
          <div className="login-item ">
            <AppButton
              disabled={!this.state.isFormValid}
              onClick={() => this.onLogin()} className="w-100 login-btn">Login now</AppButton>
          </div>
          <div className="login-item ">
            <AppButton
              onClick={() => this.onLogin()}
              className="w-100 login-google">Or sign-in with google</AppButton>
          </div>
